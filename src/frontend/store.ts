...
import {
  arcmindvectordb,
  createActor as createKnowledgebaseCanisterActor,
...
export const createStore = ({
  ...
  const getActorForCourseKnowledgebaseCanister = async (courseId: string) => {
    if (authClient) {
      const identity = await authClient.getIdentity();

      try {
        const canisterEntryResponse = await globalState.backendActor.getCourseKnowledgebase(courseId);
        // @ts-ignore
        if (canisterEntryResponse.ok) {
          ...
          // @ts-ignore
          console.error("Error retrieving course knowledgebase canister: ", canisterEntryResponse.err);
          // @ts-ignore
          throw new Error("Error retrieving course knowledgebase canister: ", canisterEntryResponse.err);
        };
      } catch (error) {
        console.error("Error in getActorForCourseKnowledgebaseCanister: ", error);
        return null;
      };
    };
    return null;    
  ...
  return {
    ...
    clearState,
    getActorForCourseKnowledgebaseCanister,
  };
};
